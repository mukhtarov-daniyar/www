...
<span class="intro">Список номеров в файле должен быть через запятую</span>
  ...
  <table class="price">
	...
	<?
		//$cdata = sqlDateNow();
        $res = getSQLArrayO("SELECT * FROM {$CFG->DB_Prefix}tmp_file_tel WHERE visible = 1 order by id DESC");
        ...
        { 
            ...
            <tr>
              ...
              <td><? if($res[$r]->status == 1){echo 'Завершено';}else {echo 'В процессе';}?></td>
       ...
       autoSubmit: true,
		   onSubmit: function() 
		   {
			...
			onComplete: function(file, response) 
			{
				...
				$(".modal-body").html('<div class="modal-header"><h4 class="modal-title"><center>Файл загружен!<br> Введите текст сообщения.<br></center></h4></div>');
				
				$(".modal-body").append('<p><input type="text" class="form-control sms" value=""></p>');
					...
					({
						...
						cache: true,			
							...
							success: function(response) 
							{
								if(response == 1)
								{
									$(".modal-body").html('<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button> <center><h3 class="modal-title">Все данные сохранены!<br> Рассылка начнется автоматически. <br>По завершению процесса вы будете извещены.</h3></center>');
