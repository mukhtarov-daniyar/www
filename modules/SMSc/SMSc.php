...
define("SMSC_LOGIN", "lidovsci");			// логин клиента
define("SMSC_PASSWORD", "4adbd696700e8ece23ff78182203a310");	// пароль или MD5-хеш пароля в нижнем регистре


//Данные устарели, перешли на другой центр
define("SMSC_LOGIN_SEPCOM", "sepcom");			// логин клиента
define("SMSC_PASSWORD_SEPCOM", "3e846e21b8dc70602c4c22e7913299e6d765086c");	// пароль или MD5-хеш пароля в нижнем регистре


//Sepcom - новый sms центр i-dgtl.ru
define("SMS_LOGIN_SEPCOM", "NTY0MTpSeUJwOFBLS0ZvNmR6SFhmWXFUQUpX"); //Ключ авторизации
define("SMS_SENDER_SEPCOM", "sms_promo"); //sms_promo
...
{
	$tphone = preg_replace('/[^0-9]/', '', $phones);
	if($tphone[1] == 7) { //Sepcom Проверка только первого номера
		...
		$m[4] = send_sms_sepcom($tphone, $message);
	}
...
$search_word = rawurldecode($arg);
$sms = explode("&", $search_word);
$str = preg_replace('~\D+~','', $sms[1]);
$str = substr($str, 1, -9);

if($str == 7)
{
	$url = (SMSC_HTTPS ? "https" : "http")."://smsc.kz/sys/$cmd.php?login=".urlencode(SMSC_LOGIN)."&psw=".urlencode(SMSC_PASSWORD)."&fmt=1&charset=".SMSC_CHARSET."&".$arg;

}
else
{
		$url = (SMSC_HTTPS ? "https" : "http")."://smsc.ru/sys/$cmd.php?login=".urlencode(SMSC_LOGIN_SEPCOM)."&psw=".urlencode(SMSC_PASSWORD_SEPCOM)."&fmt=1&charset=".SMSC_CHARSET."&".$arg;
}
...
//Sepcom start
function sms_num_check($nomer){
	//Проверка Российского номера по маске (79 и 9 цифр), проверка на стороне сервера тоже производится.
	if(preg_match("/^[7][9][0-9]{9}$/", $nomer)) {
		return true;
	}
	else {
		return false;
...
function sms_text_check($text){
	//Удаление лишних символов, на стороне сервера удаление тоже производится.
	//return preg_replace("/[^a-zA-Z0-9а-яА-Я \.\!\#\@\№\%\;\:\&\*\(\)\-\+\,\=\?\/]+/ui", "", $text);
    return str_replace( array( '\'', '"', '`' , '|' ), '', $text);
}
function send_sms_sepcom_curl($nomer, $text){
	//sms_get_ip();
	$ntext = sms_text_check($text);
	if(sms_num_check($nomer)) {

			$ch = curl_init();

			curl_setopt($ch, CURLOPT_URL, 'https://direct.i-dgtl.ru/api/v1/message');
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($ch, CURLOPT_POST, 1);
			curl_setopt($ch, CURLOPT_POSTFIELDS, "[{\"channelType\":\"SMS\",\"senderName\":\"" . SMS_SENDER_SEPCOM . "\",\"destination\":\"$nomer\",\"content\":\"$ntext\"}]");
			
			$headers = array();
			$headers[] = 'Content-Type: application/json';
			$headers[] = 'Authorization: Basic ' . SMS_LOGIN_SEPCOM;
...
function send_sms_sepcom($nomer, $text){
	$ntext = sms_text_check($text);
	if(sms_num_check($nomer)) {
		$url = "https://direct.i-dgtl.ru/api/v1/message";
		$data = ["channelType" => "SMS", "senderName" => SMS_SENDER_SEPCOM, "destination" => $nomer, "content" => $ntext]; 
		
		$content = json_encode($data);
		print_r($content);
		$curl = curl_init($url);
		curl_setopt($curl, CURLOPT_HTTPHEADER, array(
			'Authorization: Basic '. SMS_LOGIN_SEPCOM,
