...
$ACCOUNT = new Acon();
				...
				if($CFG->USER->USER_ID)
					{
						...
						$sql = "INSERT INTO {$CFG->DB_Prefix}money_accounting (data_id, author_id, user_id, type_id, cat_id, page_id, time, cdate, price, text, attachment, visible) VALUES ({$_POST[accounting]}, {$CFG->USER->USER_ID}, '{$_POST[user]}', '{$_POST[type]}', '{$_POST[cat]}', '{$CFG->oPageInfo->id}', '{$time}', '{$cdate}', '{$_POST[price]}', '{$_POST[text]}', '{$_POST[attachment]}', 1)";

						if($CFG->DB->query($sql))
						{
              $id = $CFG->DB->lastId();
							$o = getSQLRowO("SELECT price,time FROM {$CFG->DB_Prefix}money_accounting WHERE id = {$id} ");

							$status = json_encode(array('status' => 1, 'text' => "Сумма <strong>".$o->price."</strong> ".$CFG->USER->USER_CURRENCY.".<br> Код операции: <strong>".$o->time."</strong>"));
							echo $status; exit;
						}
						else
						{
			  ...
			  $director = $CFG->_GET_PARAMS[1];
				...
				if(!$data["search"] == '')
				{
					...
					if(is_numeric($search))
					{
						...
						$search_where .= " AND (text LIKE '%{$apost}%'  AND data_id = '{$director}' {$user_id}	{$types}	{$cdate}) or (price LIKE '{$search}' AND data_id = '{$director}' {$user_id}	{$types}	{$cdate})";
					...
					else
					{
						...
						{
							$apost = apost($data["search"]);
							$search_where .= " AND (text LIKE '%{$apost}%' AND data_id = '{$director}' {$user_id}	{$types}	{$cdate}) or (price LIKE '%{$apost}%' AND data_id = '{$director}' {$user_id}	{$types}	{$cdate})";
						}
					}
				}
			}
      ...
      if($_GET['pdf'] == 1)
      {
        $l = $ACCOUNT->getList($str, $search_where, 0, 0, $CFG->_GET_PARAMS[1]);
        $ACCOUNT->getListExport($l);
        exit;
      }


				echo ' <h2><img alt="" src="/tpl/img/new/icon/6_red.png">Бухгалтерия ('.$ACCOUNT->sum($CFG->_GET_PARAMS[1]).')</h2>';
...
/*
if($CFG->USER->USER_ACCES_BUCH > 0 || $CFG->USER->USER_ID == 1)
{
...
*/
    ...
    break;
			...
			default:
        $accounting = getSQLRowO("SELECT id FROM my_money_accounting_data_access WHERE user_id ='{$CFG->USER->USER_ID}'");
        if($accounting->id > 0)
        {
          include("tpl/welcome.tpl");
        }
        else
        {
          $CFG->STATUS->ERROR = true;
