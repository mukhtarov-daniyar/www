...
<style>
	...
	#accounting .col-md-6 input[type="number"]{ border-radius:3px; border:0; border:solid 1px #ccc; padding:7px 10px; font-size:18px; margin-right:10px; width:300px;}
...
<div class="row" id="accounting">
     <div class="col-md-6">
        ...
        <form method="POST">
            ...
            <label>
                <input type="number" class="price" name="search" value="" required pattern="^[ 0-9]+$">Сумма
            </label>


						<label>
                <select name="type" class="selectpicker accounting" title="Выберите кассу">
                    <?
                        $sql = getSQLArrayO("SELECT * FROM my_money_accounting_data WHERE visible=1  AND user_id = {$CFG->USER->USER_ID}  ORDER BY id ASC");
												foreach($sql as $res)
												{
            ...
            <label>
                <select name="type" class="selectpicker user" data-live-search="true">
                    <option value="0" selected><?=$CFG->Locale["fi2"]?></option>
                   <?
                    $manager = getSQLArrayO("SELECT id, name, taks_id FROM {$CFG->DB_Prefix}users WHERE visible=1 ORDER BY name ASC");
     ...
     <div class="col-md-6">
        ...
        <form class="form2" method="POST">
            ...
            <label>
                <input type="number" class="price_2" name="price" value="" required pattern="^[ 0-9]+$">Сумма в
            </label>

						<label>
                <select name="type" class="selectpicker accounting_2" title="Выберите кассу">
                    <?
                        $sql = getSQLArrayO("SELECT * FROM my_money_accounting_data WHERE visible=1  AND user_id = {$CFG->USER->USER_ID}  ORDER BY id ASC");
												foreach($sql as $res)
												{
    ...
    $('#send_data').on('click', function (e)
		{
			...
			var accounting = $('select.selectpicker.accounting').val();
        ...
        if(price > 0)
				{
					...
					({
						...
						type: "POST",
						data: {"type": type, "accounting": accounting, "price": price, "cat": cat, "user": user , "text": text , "author": author , "attachment": attachment },
						cache: true,
							beforeSend: function()
							{
							...
							success: function(response)
							{
								...
								{
									$(".modal-body").html('<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><center><h3 class="modal-title">' + response.text + '</h3></center></div>');

									$('#accounting input.price').val('');
									$('#accounting input.attachment').val('');
									$('#accounting textarea.text').val('');

									$('.selectpicker.accounting').val(0);
									$('.selectpicker.cat').val(0);
									$('.selectpicker.user').val(0);

									$('.selectpicker.accounting').selectpicker('refresh');
									$('.selectpicker.cat').selectpicker('refresh');
									$('.selectpicker.user').selectpicker('refresh');
								}
		...
		$('#send_data_2').on('click', function (e)
		{
			...
			var accounting = $('select.selectpicker.accounting_2').val();
			...
			var text = $('textarea.text_2').val();
				...
				if(price > 0)
				{
					...
					({
						url: "/accounting/record/",
						type: "POST",
						data: {"accounting": accounting, "type": type, "price": price, "cat": cat, "user": user , "text": text , "author": author , "attachment": attachment },
						cache: true,
							beforeSend: function()
							{
								$('#myModalBox').modal({backdrop: 'static', keyboard: false}) ;
