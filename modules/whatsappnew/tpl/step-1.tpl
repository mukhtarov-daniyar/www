<h2>Новая Whatsapp рассылка</h2>

<div class="white">


<?
	$res = getSQLRowO("SELECT * FROM {$CFG->DB_Prefix}tmp_whatsapp WHERE id='{$CFG->_GET_PARAMS[0]}'");
    $ser = getSQLArrayO("SELECT id FROM {$CFG->DB_Prefix}tmp_whatsapp_rss WHERE whatsapp_id='{$res->id}'");
...
.add_face { font-size: 14px; cursor: pointer;padding: 0; margin: 0; display: block; margin-bottom: 30px; text-decoration: underline; color: #999}
</style>

 <div class="kriteri" style="width:700px;">
   В данной рассылке <span style="color:#F00; display:inline-block;"><? echo count(unserialize($res->type)); ?></span> записей, из них <span style="color:#F00; display:inline-block;"><? echo count($ser); ?></span> номеров whatsapp
 </div>
...
<form method="POST" class="response">
    ...
    <input type="hidden" name="img" id="logo_input" value="/tpl/img/noavatar.jpg" />

    <div class="kriteri" style="width:700px;">
      <h3 class="add_face" data-id="<?=$CFG->_GET_PARAMS[0];?>">Добавить в рассылку критерий из "КОМПАНИЙ"</h3>
   </div>


   <script>

   	$(document).on('click','.add_face',function(e)
   	{
      ...
      $(".modal-body").append('<h4 class="modal-title"><center>Выберите критерий</center></h4>');
      $(".modal-body").append('<iframe src="/record/?company=85&ajax=yes&ajax_pids=<?=$CFG->_GET_PARAMS[0];?>" width="100%" height="500" frameborder="0" allowtransparency="true"></iframe>');

      $('.modal-body').off('click').on('click', 'button.close', function(e)
      {
   ...
   <div class="kriteri" style="width:700px;">
    <span>Тема рассылки, клиентам это не отображается</span>
      <input type="text" name="subject" id="subject" value="" placeholder="Например рассылка, банер - караганда" />
  </div>

   <div class="kriteri" style="width:700px;">
  	<span>Текст рассылки.</span>
      <textarea class="text" name="text" placeholder="Текст рассылки"></textarea>
  </div>


    <div class="kriteri" style="width:700px;">
    	<span>Отправитель.</span>
        <select name="send"<?=$e['send']?> class="selectpicker show-tick">
           <?
