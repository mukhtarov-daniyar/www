<? $CFG->oPageInfo->html_title = 'Марка авто'; ?>
<h2><img alt="" src="/tpl/img/new/icon/auto_r.png">Марка авто</h2>
...
<?
	...
	for ($i=0; $i<sizeof($datas); $i++)
    {
    	?> <li class="list-group-item">
            <strong><?=$datas[$i]->name;?></strong>
            <a href="#" class="edit" data-pos="<?=$datas[$i]->pos;?>" data-title="<?=$datas[$i]->name;?>" data-id="<?=$datas[$i]->id;?>"> изменить</a></li><?
    } ?>
        </ul>
    </article>
	&nbsp; &nbsp;<a href="#" data-id="0" class="edit"><strong>Добавить марку</strong></a>
</div>
...
<script>
	...
	$(document).on('click','.datas a.edit',function(e)
	{
		...
		{
			$(".modal-body").html('<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title">Добавить новую марку авто</h4></div>');
			$(".modal-body").append('<p><input type="hidden" class="form-control id" value="0"></p>');
			$(".modal-body").append('<p><input style="width:440px " type="tel" class="form-control text" placeholder="Название марки авто" required></p>');
			$(".modal-body").append('<p><button type="submit" class="btn btn-primary submit">Сохранить</button></p>');
		}
