...
class Seeker
{
	...
	function getCountSearch($pageId, $where="", $search_word="")
	{
		...
		return 1 * getSQLField("SELECT COUNT(id) FROM {$CFG->DB_Prefix}users WHERE {$search_word} visible='1' AND status='0' {$where}  {$and} ORDER BY cdate DESC");
	}
	...
	function getList($count=0, $start=0, $where="", $limit, $search_word="")
	{
		...
		$sql = "SELECT * FROM {$CFG->DB_Prefix}users WHERE {$search_word} visible='1' AND status='0' {$where} ORDER BY cdate DESC $w";
		$l = getSQLArrayO($sql);
		return $l;
	}
