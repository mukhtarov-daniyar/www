...
<script>
    ...
    <?
    $array = array(0, 1, 85, 133, 90, 716);
    $key = array_search($CFG->USER->USER_ID, $array); // $key = 1;

    if( $key == false) {?>
    if(id == 'good_signimpress' || id == 'default_signimpress' )
    {
      var default_signimpress = $('.skidos input[name=default_signimpress]' ).val();
      var good_signimpress = $('.skidos input[name=good_signimpress]' ).val();
      var sum = (default_signimpress*1)+(good_signimpress*1);
      if(sum >= 10)
      {
        $('#myModalBox').modal({backdrop: 'static', keyboard: false}) ;

        $(".modal-body").html('<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 style=" text-align:center;" class="modal-title">К сожалению вы не можете установить общую сумму скидок более 9%</h4></div>');
        return false;
      }
    }
    <? } ?>
