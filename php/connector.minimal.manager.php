...
$opts = array(
	...
	'roots' => array(
		...
		array(
			...
			'path'          => '../documents/filebox/',                 // path to files (REQUIRED)
			'URL'           => '../documents/filebox/', // URL to files (REQUIRED)
			'trashHash'     => 't1_Lw',                     // elFinder's hash of trash folder
			'winHashFix'    => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
			'uploadDeny'    => array('all'),                // All Mimetypes not allowed to upload
			'uploadAllow'   => array('all'), // Mimetype `image` and `text/plain` allowed to upload
			'uploadOrder'   => array('deny', 'allow'),      // allowed Mimetype `image` and `text/plain` only
			'disabled' => array('delete', 'rename', 'edit', 'hide', 'resize', 'open', 'reload', 'home', 'up', 'back', 'forward', 'getfile', 'quicklook', 'rm', 'duplicate', 'rename', 'mkfile', 'upload', 'copy', 'cut', 'paste', 'edit', 'extract', 'archive', 'info', 'view', 'help', 'new', 'move'),
		...
		*/
			...
			array(
				...
				'port' => 21,
				'alias'      => 'filebox',
				'URL'           => dirname($_SERVER['PHP_SELF']) . '/../filebox/', // URL to files (REQUIRED)
				'winHashFix'    => DIRECTORY_SEPARATOR !== '/', // to make hash same to Linux one on windows too
				'uploadDeny'    => array('all'),                // All Mimetypes not allowed to upload
				'uploadAllow'   => array('all'), // Mimetype `image` and `text/plain` allowed to upload
				'uploadOrder'   => array('deny', 'allow'),      // allowed Mimetype `image` and `text/plain` only
				'disabled' => array('delete', 'copy'),
//				'disabled' => array('delete', 'rename', 'edit', 'hide', 'resize', 'open', 'reload', 'home', 'up', 'back', 'forward', 'getfile', 'quicklook', 'rm', 'duplicate', 'rename', 'mkfile', 'upload', 'copy', 'cut', 'paste', 'edit', 'extract', 'archive', 'info', 'view', 'help', 'new', 'move'),
				'accessControl' => 'access'
			),
